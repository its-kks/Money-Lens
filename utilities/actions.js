    ...
    try {
      ...
      for (let i = 0; i < recurringPayments.length; i++) {
        ...
        if (amount > 0) {
          ...
          if (action_added[1] != todays_date.getMonth() + 1 &&
            pay_date[0] == todays_date.getFullYear() &&
            pay_date[1] == (todays_date.getMonth() + 1)
            && todays_date.getDate() >= pay_date[2]) {

            dispatch(addActionRequest({ actionAmount: amount, actionType: 'Add', recurringPaymentID: id }));
